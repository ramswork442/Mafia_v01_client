    ...
    <div className="bg-gray-900 bg-opacity-90 p-4 sm:p-6 rounded-xl shadow-xl border border-gray-700">
      ...
      <ul className="space-y-2 sm:space-y-3">
        {displayPlayers.map((player, index) => {
          ...
          const isGodfather = player.role === 'Godfather' && isMafia && isGangMember;
          const isRegularMafia = player.role === 'Mafia' && isMafia && isGangMember;

          return (
            <li
              key={player.name} // Use name for stable key
              className={`flex items-center gap-2 sm:gap-3 text-white ${player.name === currentPlayer ? 'font-bold text-yellow-300' : ''} ${!player.isAlive ? 'line-through text-gray-500' : ''}`}
            >
              <span className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full ${player.isReady ? 'bg-green-500' : 'bg-red-500'} shadow-md`}></span>
              <span className="text-xs sm:text-sm">{player.name}</span>
              {player.name === currentPlayer && playerRole && (
                <span className="text-gray-300 ml-1 sm:ml-2 text-xs sm:text-sm">— {playerRole}</span>
              )}
              {(isMafia || isDead) && isGangMember && (
                ...
                >
                  {isGodfather ? '[Godfather]' : isRegularMafia ? '[Mafia]' : ''}
                </span>
              )}
              {!player.isAlive && (
                <span className="ml-1 sm:ml-2 text-gray-500 text-xs sm:text-sm">[Dead]</span>
              )}
            </li>
          );
        })}
